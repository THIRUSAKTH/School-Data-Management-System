...
class _ParentNotificationsPageState extends State<ParentNotificationsPage> {
  ...
  Future<void> _loadStudents() async {
    ...
    try {
      ...
      if (snapshot.docs.isNotEmpty) {
        _processStudents(snapshot.docs);
      } else {
        setState(() {
  ...
  void _processStudents(List<QueryDocumentSnapshot> docs) {
    if (docs.isEmpty) {
      setState(() {
        _isLoading = false;
        _errorMessage = "No students linked to your account.";
      });
      return;
    }
    ...
    final Map<String, dynamic> data = selectedDoc.data() as Map<String, dynamic>;
    final name = (data['name'] ?? 'Student').toString();

    setState(() {
      _selectedStudentId = selectedDoc.id;
      _studentName = name;
      _isLoading = false;
    });
  }
  ...
  Widget _buildNotificationsList() {
    return Column(
      children: [
        if (_studentName != null) _buildStudentInfo(),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              "Showing notifications for: $_studentName",
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        return Container(
          ...
          child: Row(
            children: [
              const Icon(Icons.switch_account, size: 18, color: Colors.orange),
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    items: students.map((student) {
                      final Map<String, dynamic> data = student.data() as Map<String, dynamic>;
                      final String name = (data['name'] ?? 'Student').toString();
                    ...
                    onChanged: (value) {
                      setState(() {
                        ...
                        final QueryDocumentSnapshot selectedStudent = students.firstWhere((s) => s.id == value);
                        final Map<String, dynamic> data = selectedStudent.data() as Map<String, dynamic>;
                        _studentName = (data['name'] ?? 'Student').toString();
                      });
                    },
                  ),
                ),
  ...
  Widget _buildNotificationCard(String notificationId, Map<String, dynamic> data) {
    ...
    Map<String, dynamic> typeConfig = _getTypeConfig(type);
              ...
              child: Row(
                children: [
                  CircleAvatar(
                    ...
                    child: Icon(typeConfig['icon'] as IconData, color: typeConfig['color'] as Color),
                  ),
